	...
	class Give_Recurring_Subscription_Payment_Failed_Email extends Give_Email_Notification {
		...
		public function get_setting_fields( $form_id = null ) {
			...
			$supported_gateways_field[] = array(
				...
				'default'    => $this->default_active_payment_gateways(),
				'attributes' => array(
		 ...
		 *
		 * @since  1.9.0
		 * @since 1.12.2 Set email recipient to donor email
		 * @access public
		 *
		 * @return void
		 */
		 ...
		 * @param  Give_Recurring_Subscription_Payment_Failed_Email  $email  Email.
		 *
		 * @return string
		 */
		...
		/**
		 * Return list of default active payment gateway (only stripe except stripe ideal).
		 *
		 * @since 1.12.2
		 * @return array
		 */
		private function default_active_payment_gateways(){
			$supported_gateways   = wp_list_pluck( give_get_enabled_payment_gateways(), 'admin_label' );
			...
			foreach ( $supported_gateways as $gateway_slug => $gateway_name ) {
				if (
					strpos( $gateway_slug, 'stripe' ) !== 0 ||
					'stripe_ideal' === $gateway_slug
				) {
