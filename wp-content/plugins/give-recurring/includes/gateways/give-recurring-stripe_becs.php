...
class Give_Recurring_Stripe_Becs extends Give_Recurring_Gateway {
	...
	public function create_payment_profiles() {
		...
		if ( ! empty( $stripe_customer_id ) ) {
			$payment_method = $this->stripe_customer->attached_payment_method;
	...
	public function subscribe_customer_to_plan( $stripe_customer, $source, $plan_id ) {
		if ( $stripe_customer instanceof \Stripe\Customer ) {
			try {
			    // Set the subscription as `Offsite` so that we can keep the subscription `Pending`.
			    $this->offsite = true;
				...
				// We have set subscription as `Active` so that webhooks work properly.
