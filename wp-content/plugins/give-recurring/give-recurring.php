...
if ( ! defined( 'GIVE_RECURRING_VERSION' ) ) {
	define( 'GIVE_RECURRING_VERSION', '1.8.13' );
}
if ( ! defined( 'GIVE_RECURRING_MIN_GIVE_VERSION' ) ) {
	define( 'GIVE_RECURRING_MIN_GIVE_VERSION', '2.4.0' );
}
...
final class Give_Recurring {
	...
	public function init() {
		...
		self::$gateways = apply_filters( 'give_recurring_available_gateways', array(
	...
	private function includes_global() {
		...
		foreach ( give_get_payment_gateways() as $gateway_id => $gateway ) {
			...
			// Prepare file path.
			$file_url = sprintf(
				/* translators: 1. Plugin Directory Path, 2. Gateway ID. */
				'%1$s/includes/gateways/give-recurring-%2$s.php',
				GIVE_RECURRING_PLUGIN_DIR,
				$gateway_id
			);

			// Proceed only, if gateway is active and file exists.
			if (
				give_is_gateway_active( $gateway_id ) &&
